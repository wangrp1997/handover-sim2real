...
### OMG-Planner Compilation Issues

If you encounter compilation errors when installing the `OMG-Planner` submodule, particularly with `ycb_render`, the following fixes may help:

#### 1. CMake Version Compatibility Issues

**Problem**: CMake errors like "Compatibility with CMake < 3.5 has been removed"

**Solution**: Update the minimum CMake version in the following files:

```bash
# Update CMakeLists.txt files
cd OMG-Planner/ycb_render
sed -i 's/cmake_minimum_required(VERSION 2.8.12)/cmake_minimum_required(VERSION 3.5)/g' CMakeLists.txt
sed -i 's/cmake_minimum_required(VERSION 2.8.12)/cmake_minimum_required(VERSION 3.5)/g' pybind11/CMakeLists.txt
sed -i 's/cmake_minimum_required(VERSION 2.8.12)/cmake_minimum_required(VERSION 3.5)/g' pybind11/tools/pybind11Tools.cmake
```

#### 2. pybind11 Version Compatibility Issues

**Problem**: C++ compilation errors with Python 3.10+ like "'uint16_t' in namespace 'std' does not name a type"

**Solution**: Upgrade pybind11 to a compatible version:
...
**Problem**: Compilation errors like "'cout' is not a member of 'std'"

**Solution**: Add missing header includes to source files:

```bash
# Add #include <iostream> to ycb_renderer.cpp if needed
echo '#include <iostream>' | sed -i '18i\\' cpp/ycb_renderer.cpp
```

#### 4. PointNet2 CUDA Extension Issues

**Problem**: CUDA kernel errors like "no kernel image is available for execution on the device"

**Solution**: Recompile PointNet2 with your GPU architecture:

```bash
cd GA-DDPG/Pointnet2_PyTorch/pointnet2_ops_lib
pip uninstall -y pointnet2_ops || true
python setup.py clean && rm -rf build *.egg-info
export TORCH_CUDA_ARCH_LIST="8.9;9.0;10.0+PTX"  # For RTX 50 series
export CUDA_HOME=/usr/local/cuda-12.8
pip install -v --no-deps -e .
```
...
```bash
# Test PointNet2 CUDA extension
python3 -c "import torch; from pointnet2_ops import pointnet2_utils as p2; xyz=torch.randn(2,1024,3,device='cuda',dtype=torch.float32); idx=p2.furthest_point_sample(xyz,128); print('PointNet2 OK:', idx.is_cuda)"

# Test ycb_render compilation
cd OMG-Planner/ycb_render && python setup.py develop
```

These fixes address common compatibility issues when building on modern systems with Python 3.10+ and CUDA 12.x.

#### 5. AttributeError: 'ActorWrapperRemote' object has no attribute '_env'
**Problem**: pyassimp.errors.AssimpError: assimp library not found
**Solution**: 
```bash
https://blog.csdn.net/qq_44949041/article/details/131897097
```

**Problem**: ImportError: /home/rw/miniconda3/envs/handover/bin/../lib/libstdc++.so.6...
**Solution**: 
```bash
